...
def _Init():
	...
	if is_win:
		...
		import os as _os
		# Walk up from this file's directory to find mindvision_sdk/lib/x64/libMVSDK.so
		# (works whether the package is installed or run from the project tree)
		_lib_name = "libMVSDK.so"
		_arch = "x64" if _os.uname().machine == "x86_64" else \
		        "arm64" if _os.uname().machine == "aarch64" else \
		        "arm"
		_search_dir = _os.path.dirname(_os.path.abspath(__file__))
		_found = None
		for _ in range(8):
			_candidate = _os.path.join(_search_dir, "mindvision_sdk", "lib", _arch, _lib_name)
			if _os.path.exists(_candidate):
				_found = _candidate
				break
			_search_dir = _os.path.dirname(_search_dir)
		_sdk = cdll.LoadLibrary(_found if _found else _lib_name)
