...
class robot:
    def __init__(self, robotIP: str, DEBUG: bool=False):
        ...
        # Retry the initial EtherNet/IP reads so nodes survive a slow robot boot
        # or a brief network hiccup at launch time.
        _retries = 10
        _delay   = 3.0
        for _attempt in range(1, _retries + 1):
            try:
                ...
                if _attempt < _retries:
                    print(f"[robot_controller] connect attempt {_attempt}/{_retries} failed "
                          f"({_exc}) — retrying in {_delay:.0f}s")
                    time.sleep(_delay)
                else:
                    raise
